...
from paper_analysis.pdf_text_tables import build_figure_summary, build_llm_context, load_text_artifacts, run_text_dump
...
@app.command("extract-combined")
def extract_combined_cmd(
    config: Path = typer.Option(Path("config/poc.yaml"), "--config", "-c", exists=True),
) -> None:
    """LLM pass: extract runs + measurements together in one call, writing candidate_combined.json.

    Produces a denormalized table where every measurement row carries full run
    metadata (experiment_type, strain, plasmid, medium, etc.), making
    experiment-to-data linking automatic.  Requires extract-text first;
    for best results, also run extract-figures + run-vision first so figure
    data is included in the context.
    """
    cfg = _load_cfg(config)
    tc = effective_text_config(cfg)
    try:
    ...
    if fig_summary:
        content = content + "\n\n" + fig_summary
        typer.echo(f"Including figure extraction data from {cfg.artifacts.extractions_dir}")
    else:
        typer.echo(
            "No figure extractions found; run extract-figures + run-vision first "
            "for best coverage. Proceeding with text-only context."
        )
    provider = cfg.vision.provider or os.environ.get("PAPER_ANALYSIS_VISION_PROVIDER", "anthropic")
    model = tc.llm_model or cfg.vision.model
    batch = run_combined_analysis(content, provider=provider, model=model, max_retries=2)
    out = tc.output_dir / "candidate_combined.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(batch.model_dump_json(indent=2), encoding="utf-8")
    n_runs = len({c.run_id for c in batch.candidates})
    typer.echo(f"wrote {out} ({len(batch.candidates)} measurements across {n_runs} runs)")
